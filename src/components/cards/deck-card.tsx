  ...
  return (
    <div className="group flex flex-col rounded-xl border border-gray-200 bg-white p-5 shadow-sm transition-shadow hover:shadow-md">
      ...
      <div className="flex items-start gap-3">
        <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-lg bg-indigo-50 text-lg font-bold text-indigo-600">
          {deck.name.charAt(0).toUpperCase()}
        </div>
        <div className="min-w-0 flex-1">
          <Link href={`/decks/${deck.id}`}>
            <h3 className="truncate font-semibold text-gray-900 hover:text-indigo-600">
              {deck.name}
            </h3>
          </Link>
          {deck.description ? (
            <p className="mt-0.5 line-clamp-2 text-sm text-gray-500">{deck.description}</p>
          ) : (
            <p className="mt-0.5 text-sm text-gray-400 italic">Sem descrição</p>
          )}
        </div>
      </div>
